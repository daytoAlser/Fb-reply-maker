...
- If you reference a specific product in your reply, it MUST be one of the items above. Do NOT invent SKUs, model names, prices, or stock claims. Common hallucination pattern: when the customer pushes back on price, you may feel pulled toward naming a "budget tire" like Haida, Sailun, Wanli, or similar tertiary brands. DO NOT — those aren't in our live inventory and naming them is a hard fail. If a cheaper SKU exists in the items above, use that. If not, ask the customer about size flexibility instead.
- Use the availability framing from each item ("ready to rock" / "we can get those for ya"). NEVER say "in stock" or pin to a specific location — ABSOLUTE RULE D2 still applies.
...
- WHEN THIS RULE FIRES, the install question ("you looking at install too or just the tires?") is FORBIDDEN in every variant. The customer's open question about wheels is unanswered without vehicle info, so the variant's CTA must be the vehicle-for-rims ask. Install gets answered later, after the rim flow gets going.
- ALL THREE VARIANTS (Quick, Standard, Detailed) MUST end with EXACTLY one rim-vehicle ask. No exception for Quick being short. The rim ask is the closing line.
- Quick variant closing rim ask EXAMPLES (pick one, never the install question):
  - "And for the rims, what year/make/model are we working with?"
  - "What's the vehicle for the rim fitment?"
- Standard / Detailed closing rim ask EXAMPLES:
  - "And on the rim side, what year/make/model are we building this for?"
  - "For the rims, I'll need year/make/model so I can match the bolt pattern."
- HARD FAIL examples (never produce these when wheels are in scope):
  - "...$131.75 each. You looking at install too or just the tires?" (install ONLY, no rim ask — FORBIDDEN)
  - "...$131.75 each. You looking at install too or just the tires? And on the rim side, what year/make/model..." (BOTH install + rim ask in same variant — STILL FORBIDDEN, drop install)
- The install question is RESERVED for TIRE-ONLY conversations (no rims/wheels mentioned anywhere in message or history).
...
`;
  ...
  // Known warranty fact: iLink non-winter tires carry a 60,000 km tread
  // warranty. Surface this to the AI when the lead pick qualifies so it
  // can state the warranty directly instead of deflecting ("the rep will
  // know") — we ARE the rep. For any other case (non-iLink brand, or
  // winter-only iLink, or no lead) the AI is instructed to OMIT warranty
  // mentions entirely rather than guess.
  const leadIsIlinkNonWinter = !!(liveLeadProduct
    && /\bilink\b/i.test((liveLeadProduct.brand || '') + ' ' + (liveLeadProduct.name || ''))
    && !liveLeadProduct.winterOnly);
  const knownWarrantyClause = leadIsIlinkNonWinter ? '60,000 km tread warranty' : null;
...
6. CTA — depends on whether install has been answered yet (installAnswer.state = "${installAnswer.state}"${(installAnswer.state === 'answered' || installAnswer.state === 'followed_up') ? `, affirmative=${installAnswer.affirmative}${installAnswer.soft ? ', signal=implicit (customer asked install-progression questions like "do you install? balance free? alignment?")' : ''}` : ''}):
   ${installAnswer.state === 'answered' && installAnswer.affirmative === true
     ? `INSTALL ANSWERED AFFIRMATIVE${installAnswer.soft ? ' (IMPLICIT — customer asked install-progression questions like "do you install? balance? alignment?" rather than saying "yes"; treat as committed)' : ''}. The rep has NOT yet followed up with phone-for-estimate. Do NOT ask the install question again.${installAnswer.soft ? ` FIRST: in 1 short clause per question, answer any substantive questions the customer asked (install: "yep we install", balance: "balance included with install", alignment: "alignment we can add on"${knownWarrantyClause ? `, warranty: "iLink covers it with a ${knownWarrantyClause}" (state this only — do NOT say "multi-year" or "sales rep has the exact length")` : ', warranty: OMIT — if the customer asked about warranty and we do not have a verified figure, do NOT mention warranty at all. NEVER say "the rep has the exact length" or "sales rep will know" — WE ARE THE REP'}, "lowest price": stay at the sticker — no discount). Combine concisely; do not turn into a paragraph.` : ''} ${installAnswer.soft ? 'THEN' : ''} Pivot to PHONE-FOR-ESTIMATE: "Wicked — send me your phone number and we'll get the formal estimate over with everything (tires + install + taxes${installAnswer.soft ? ' + balance + alignment' : ''}) broken down line by line." / "Awesome, send a good phone number and we'll have the full estimate shot over broken down — tires, install, all in." Variants must end with this phone ask, NOT "when do you want it booked" or "when are you free to come in" — that's premature; phone first, scheduling after the estimate is in their hands. FORBIDDEN: "when would you want to get them on the truck", "when are you looking to book that in", "when's good for you to come in", any scheduling question. FORBIDDEN voicing: "sales rep has", "the rep will know", "let me check with the rep" — WE ARE THE REP, never deflect knowledge to a third party.`
     : installAnswer.state === 'answered' && installAnswer.affirmative === false
       ...
       : installAnswer.state === 'followed_up'
         ? `INSTALL ALREADY ANSWERED AND THE PHONE-FOR-ESTIMATE PIVOT HAS ALREADY BEEN MADE. DO NOT re-ask install. DO NOT re-ask for phone number. DO NOT re-pitch the product. Respond naturally to whatever the customer's latest message actually says — if they sent a phone number, acknowledge it and confirm we'll get the estimate over. If they're asking about scheduling/timing, say "we'll lock in timing once you've got the estimate" (NEVER "the sales rep will sort timing" — WE ARE THE REP). If they're chitchatting or asking something unrelated, just answer the question without a sales CTA.`
      ...
      if (priceObjection.triggered) {
        ...
        if (installAnswer.soft) {
          const warrantyClause = knownWarrantyClause
            ...
            : ', warranty: OMIT entirely — if the customer asked about warranty and we do not have a verified figure for THIS product, do NOT mention warranty. NEVER say "the rep has the exact length" or "sales rep will know" — WE ARE THE REP, never deflect to a third party';
          ctaInstruction = `Install has been answered AFFIRMATIVE (IMPLICIT — customer asked install-progression questions like "do you install? balance free? alignment?" instead of saying "yes"; treat as committed). DO NOT re-ask install. FIRST: answer the customer's substantive questions in 1 short clause each (install: "yep we install", balance: "balance is included with install", alignment: "alignment we can add on when you're in"${warrantyClause}). Keep it tight — combine into one sentence if you can. DO NOT discount the sticker price if they asked "lowest price". THEN close with the PHONE NUMBER ask: "Send me your phone number and we'll have the full estimate over — tires, install, balance, alignment, taxes all broken down." FORBIDDEN: asking install again, asking timing/scheduling, dropping the sticker, deflecting facts to "the rep".`;
        } else {
          ctaInstruction = `Install has been answered AFFIRMATIVE (yes) — do NOT ask install again. CLOSE every variant by asking for the PHONE NUMBER so a formal estimate (tires + install + taxes) can be sent. Example phrasing: "Send me your phone number and we'll get the full estimate over broken down line by line." FORBIDDEN this turn: asking install again, asking "when do you want it booked", asking "when's good for you" — scheduling fires AFTER the estimate is delivered.`;
        }
      } else if (installAnswer.state === 'answered' && installAnswer.affirmative === false) {
        ctaInstruction = `Install has been answered NEGATIVE ("just tires"). CLOSE every variant by asking for the PHONE NUMBER so a tire-only estimate can be sent. Example: "Send me your phone number and we'll get the formal estimate over with the tires + taxes broken out." FORBIDDEN this turn: asking install again, asking when they want install.`;
      } else if (installAnswer.state === 'followed_up') {
        ctaInstruction = `Install has already been answered AND the phone-for-estimate pivot has ALREADY been made. DO NOT re-pitch the product. DO NOT re-ask install. DO NOT re-ask for phone number. DO NOT add a sales CTA. Respond naturally to whatever the customer's current message actually says — if they sent a phone number, acknowledge briefly. If they're asking timing/scheduling, say "we'll lock in timing once you've got the estimate" (NEVER "sales rep will sort timing" — WE ARE THE REP). Keep variants short and conversational, no product re-pitch.`;
      } else {
